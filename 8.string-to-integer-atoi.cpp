 ...
 public:
  int myAtoi(string s) {
    ...
    for (auto it = s.begin(); it != s.end(); ++it) {
      if (isdigit(*it)) {
        if (!catch_int && res_str.size() == 0) { catch_int = true; }
        if (catch_int) { res_str.push_back(*it); }
      } else if (isspace(*it) && !catch_int) {
      } else if ((*it == '-' || *it == '+') && !catch_int) {
        if (!isdigit(*(it + 1))) { break; }
        if (*it == '-') { sign = -1; }
      } else if (catch_int && !isdigit(*it)) {
        break;
      } else if (!isdigit(*it) && (!isspace(*it) || *(it) != '+' || *(it - 1) != '-')) {
        break;
      }
    }
    ...
    res_str = res_str.substr(real_start_index);
    long int res_int = 0;
    if (res_str.size() > std::to_string(INT32_MAX).size()) {
      if (sign == 1) {
        res_int = pow(2, 31) - 1;
      } else {
        res_int = -pow(2, 31);
      ...
      for (int i = res_str.size() - 1; i >= 0; --i) {
        res_int += pow(10, (res_str.size() - 1 - i)) * (res_str[i] - '0');
      }
      res_int *= sign;
      if (res_int > (pow(2, 31) - 1)) {
        res_int = pow(2, 31) - 1;
      } else if (res_int < -pow(2, 31)) {
        res_int = -pow(2, 31);
      }
    }
    return res_int;
