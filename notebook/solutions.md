...
路径是vector<string>，只记录分割的子串，不包含逗点

终止条件：路径长度等于4（根据ip地址的格式），字符串总长度等于输入字符串的长度

若满足终止条件即遇到了解，将子字符串数组转化为ip地址字符串，再放入结果数组中

剪枝条件：

for循环外：若路径长度大于4,或剩余字符的数量/3大于剩余可添字段的数量，直接return

for循环内：若字段字串长度大于1,且第一位为0, 或子串转成int的值不在[0, 255]，则不合法，直接return

for循环，循环的是子字符串长度，从1到s.size()-start_index和3之间的最小值(3是最大合法字串255的长度)
...
集合问题：要把所有节点添加到最终的解中，而非只添加叶子节点（组合，分割问题）

终止条件：无

for循环：i范围：[start_index, nums.size())，

path加上nums[i]，

backtracking，start_index = i + 1，

最终主函数返回解的时候，不要忘记添加空集，因为空集是所有集合的子集

## 84. #90 子集Ⅱ

题目78的变种，需要处理同层的重复问题，沿同一条路径，也就是深度不同的重复元素可以被接受，但是同层的不能被接受，因为这样会造成重复解

对输入数组排序，保证若有重复数字，都会紧邻着彼此

用到used数组，记录数组中的每个元素在当前路径中有没有出现过

剪枝条件：nums的前一个与当前元素值相等，但是上一个元素没有在当前路径中出现过，那么就continue，略过这个节点

其他与78题基本相同，除了处理节点的时候，要处理path和used

## 85. #491 递增子序列

终止条件：无

由于不能排序，要保持输入数组的顺序，所以重复元素可能并非相邻着出现，使用一个不同的used数组：

大小为201，可能出现的元素大小从-100到100, used数组记录这201个数字在同一层是否出现过

因此在每一次递归内，for循环前，初始化used数组结尾false，这样能保证相同元素值能添加不同深度的节点

剪枝条件：for循环内：若当前值小于path的最后一个元素值，或者这个元素在这一层出现过（检查used数组），continue

backtracking，start_index = i + 1，

注意，used只从false更新true，不会随着path popback的同时由true变回false，因为维护的是一层中元素是否出现

## 86. #46 全排列

排列问题

用used数组来监测路径中元素的使用情况

终止条件：路径长度等于输入数组长度

剪枝条件：for循环内，若当前元素被使用过（检查used数组），continue

for循环，nums的index [0, num.size()]
...
剪枝条件多一个：若当前元素与上一元素相等，且上一元素不再路径中，continue
...
path：int vector，记录皇后在棋盘中的位置，index代表行数，元素值代表列数

终止条件：path size等于输入的n，将path转换成合法的形式后加入res数组

剪枝条件：出现同列或同斜线的皇后，continue。

    同列判断：当前希望被添加的位置（列数）在path中出现过，

    斜线判断：当前皇后与path中的任一皇后组成的斜线是正或负45度，也就是两个皇后的行数差等于正或负的列数差

    同行判断：由于行数是index，所以天然不会重复，无需特别判断

    判断剪枝需要遍历path，若出现不满足要求的位置就可以break掉遍历循环，然后continue到下一个候选位置

for循环：循环可行列数：[0, n]

backtracking参数不变，因为都要从0开始遍历

## 90. #37 解数独
棋盘问题，二维

bool backtracking(board)：由于得到了一组解就返回即可，所以回溯函数有返回布尔值

二维循环棋盘，若当前元素没有填充数字，就从1至9检查其填充相应数字的可能性，若某个数字可以在该位置被填充，那就给棋盘的该位置赋值，然后调用backtracking函数(参数棋盘已被更新)，如果返回值为true，那么当前函数就返回true;

如果所有9个值遍历完仍没有解，就返回false

如果二维循环全部遍历完，就返回true，证明已找到解

## 91. #912 排序树组
1. 冒泡排序，Time: O(n^2)  Space: O(1) 稳定

遍历数组，比较相邻元素，若出现前一项大于后一项就交换。

第一次遍历n个元素，可以保证最大的元素在最后一位

第二次遍历前n-1个元素，可以保证第二大的元素在倒数第二位

。。。

最后一次遍历1个元素

共比较n(n-1)/2次，时间复杂度O(n^2)，空间复杂度O(1)

最快：正序：O(n)，最慢：逆序：O(n^2)，平均：O(n^2)

优化：用一个布尔值记录每一次遍历内是否发生元素交换，若没有，即说明已经完成排序，可以立即停止排序了

2. 选择排序，Time: O(n^2)  Space: O(1) 不稳定

遍历数组，index i从0到n，

每次寻找从i到n中最小的元素，交换最小元素和i处的元素

共比较n(n-1)/2次，最好最坏的时间复杂度都是O(n^2)，且不稳定

优化：记最小的同时记最大，同时更新左右两个极值，减小一般的时间

3. 插入排序，Time: O(n^2)  Space: O(1) 稳定

遍历数组，index i从1到n，

每次再从i开始向前依次比较前后两个相邻元素，若前一个大于后一个，就交换两个元素，直到第一个元素
...
优化：
    ...
    拆半插入：每一次插入过一个元素后，可以保证从0到这个元素原本位置的所有元素是排好序的，

    那么在选择下一个元素的插入位置时，可以使用二分法，

    这样比较的次数可以简化为O(nlogn)，但是结合元素的移动（需要移动一半的元素），整体的的时间复杂度仍在O(n^2)

4. 希尔排序，Time: O(n^(1.3-2))  Space: O(1) 不稳定

希尔排序是插入排序的改进

观察插入排序，可以发现，当数组接近为正序的时候，最好可达到O(n)的时间复杂度

希尔排序通过选择不同的gap来对数组中的一部分进行选择排序，

比如gap = 4，那就比较位于第0，4，8 。。。的元素

gap的选择有若干种方法，可以选择gap = 3*gap + 1 (knuth法);

gap大的时候，数组顺序还比较混乱，可能发生的比较和交换的次数多

随着gap减小，数组逐渐像正序逼近，可能发生的比较和交换次数少

直到gap=1，也就是依次比较相邻元素，变成插入排序，但是由于前面的若干次排序，此时只需要O(n)的时间复杂度

gap按从大到小，直至唯一的顺序，对数组进行若干次遍历

每次遍历内，对选择出的若干个元素做插入排序

希尔排序的时间复杂度，取决于gap的选择，

对于Knuth方法，最坏（逆序）的时间复杂度是O(n^3/2)，

最好（正序）的是O(n^5/4)

5. 归并排序，Time: O(nlogn)  Space: O(n) 稳定

分治的思想，用递归法解决

排序一个长数组，可以先分别排序它的左右两个数组，然后再把左右两个数组合并在一起

递归终止条件：数组长度为1，那么直接返回单元素的数组，或者长度为2，那么返回排序后的二元素数组

在每次递归内，先分别获取左右两个子数组，然后再把两个数组合并在一起：

合并时，用两个指针分别指向左右两个数组的元素，哪个小选哪个，并更新相应的指针，

直到一个数组被遍历完，把另一个数组的剩余元素依次填到结果中即可，

返回结果。

最好最坏平均的时间复杂度相同都是O(nlogn)，空间复杂度O(n），每次递归内需要有一个临时的结果数组
